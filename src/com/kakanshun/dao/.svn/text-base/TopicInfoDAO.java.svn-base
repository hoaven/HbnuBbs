...
public class TopicInfoDAO {
	 ...
	 * @param tid
	 *            主帖编号
	 * @return DetailPage 返回一个对象（封装主帖和用户信息）
	 */
...
//				+ " from topicInfo where tId = ?";
		String sql = "select * from gettopicinfoanduserInfo where tId =?";
		try {
...
//				+ "from topicInfo where tsid = ? order by tPublishTime desc";
		String sql = "select * from gettopicinfo where tsid = ? order by tPublishTime desc";
