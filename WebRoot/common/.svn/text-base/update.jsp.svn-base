...
<DIV><BR/>
	...
	%>
	<DIV>
		<FORM name="updateForm" onSubmit="return check(1);" action="${pageContext.request.contextPath}/servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&action=updateTopic" method="post">
	...
	%>
		<DIV>
		<FORM name="updateForm" onSubmit="return check(0);" action="${pageContext.request.contextPath}/servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&rid=<%=editreply.getRid()%>&action=updateReply" method="post">
			<INPUT type="hidden" name="boardId" value="8"/>
			<INPUT type="hidden" name="replyId" value="61"/>
			<DIV class="t">
				<TABLE cellSpacing="0" cellPadding="0" align="center">
