...
<%@ page import="java.util.*,com.kakanshun.page.*,com.kakanshun.dao.*,com.kakanshun.domain.*"%>
<%
...
<Link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/common/style/style.css" />
</HEAD>
...
<DIV><br/>
	<!--      导航        -->
<DIV>
	&gt;&gt;<B><a href="${pageContext.request.contextPath}/common/index.jsp">论坛首页</a></B><%@ include file="navigation.jsp" %>
</DIV>
	<br/>

	<!--      回复、新帖        -->
	<DIV>
		<A href="${pageContext.request.contextPath}/common/post.jsp?tid=<%=tid%>&sid=<%=sid%>&action=reply"><IMG src="${pageContext.request.contextPath}/common//image/reply.gif"  border="0" id=td_post></A> 
		<A href="${pageContext.request.contextPath}/common/post.jsp?tid=<%=tid%>&sid=<%=sid%>&action=post"><IMG src="${pageContext.request.contextPath}/common/image/post.gif"   border="0" id=td_post></A>
	</DIV>

	<!--         翻 页         -->
	<DIV class="pages">
	
	<a href="${pageContext.request.contextPath}/servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&action=showPage&currPage=<%=--PreviousPage%>">上一页</a>&nbsp;|&nbsp;
		<a href="${pageContext.request.contextPath}/servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&action=showPage&currPage=<%=++nextPage%>">下一页</a>&nbsp;|&nbsp;
	<%
		if(PageCount > 0){
	%>
		<a href="${pageContext.request.contextPath}/servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&action=showPage&currPage=1"><<</a>&nbsp;
	...
	%>
		<a href="${pageContext.request.contextPath}/servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&action=showPage&currPage=<%=i%>"><%=i%></a>&nbsp;
	<%
		} 
	%>
	<%-- 实现分页 打印代码一 结束 --%>
		...
		<a href="${pageContext.request.contextPath}/servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&action=showPage&currPage=<%=PageCount%>">>></a>&nbsp;
	...
	<DIV class="t">
		...
		<TH style="WIDTH: 20%">
			...
			<img src="${pageContext.request.contextPath}/common/image/head/<%=topicObj.getFace()%>"/><BR/>
		...
		<TH>
			...
			%>
			 <A href="${pageContext.request.contextPath}/servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&action=delTopic"
			...
			%>
				<A href="${pageContext.request.contextPath}/servletDetailPage?tid=<%=tid%>&sid=<%=sid%>
	...
	<TH style="WIDTH: 20%">
		...
		<img src="${pageContext.request.contextPath}/common/image/head/<%=replyList.get(i).getFace()%>"/><BR/>
		...
		<DIV class="tipad gray">
			...
			<%
			 ...
			 <%
				...
				if(users != null && users.getUtype() == 2 ){ 
			%>
			<A href="${pageContext.request.contextPath}/servletDetailPage?tid=<%=tid%>&sid=<%=sid%>&rid=
